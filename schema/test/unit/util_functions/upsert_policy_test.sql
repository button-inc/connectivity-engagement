...
select has_function(
  'connectivity_engagement_private', 'upsert_policy',
  'Function upsert_policy should exist'
);

select throws_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_select_all', 'test_table', 'badoperation', 'connectivity_engagement_admin', 'true');
  $$,
  'P0001',
...
select throws_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_select_all', 'test_table', 'select', 'connectivity_engagement_admin', 'using(true)', 'with check(true)', 'connectivity_engagement');
  $$,
  'P0001',
...
select lives_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_select', 'test_table', 'select', 'connectivity_engagement_admin', 'true');
  $$,
...
select lives_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_delete', 'test_table', 'delete', 'connectivity_engagement_admin', 'true');
  $$,
...
select lives_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_insert', 'test_table', 'insert', 'connectivity_engagement_admin', 'true');
  $$,
...
select lives_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_update', 'test_table', 'update', 'connectivity_engagement_admin', 'true');
  $$,
...
select policies_are(
    ...
    'The correct policies ahve been created for connectivity_engagement.test_table'
);
...
select lives_ok(
  $$
    select connectivity_engagement_private.upsert_policy('admin_delete', 'test_table', 'delete', 'connectivity_engagement_admin', 'false');
  $$,
...
select lives_ok(
  $$
    select connectivity_engagement_private.upsert_policy(
      'different_statement_update', 'test_table', 'update', 'connectivity_engagement_admin', 'using(true)', 'with check(false)', 'connectivity_engagement'
    );
  $$,
