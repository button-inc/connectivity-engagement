...
select has_function(
  'connectivity_engagement_private', 'verify_grant',
...
select lives_ok(
  $$
    select connectivity_engagement_private.verify_grant('all', 'test_table', 'test_role');
  $$,
  'verify_grant passes when all permissions granted to user'
);

revoke all on table connectivity_engagement.test_table from test_role;
grant select on table connectivity_engagement.test_table to test_role;

select lives_ok(
  $$
    select connectivity_engagement_private.verify_grant('select', 'test_table', 'test_role');
  $$,
  'verify_grant passes when one permission granted to user'
);

revoke all on table connectivity_engagement.test_table from test_role;
grant all (allowed) on table connectivity_engagement.test_table to test_role;

select lives_ok(
  $$
    select connectivity_engagement_private.verify_grant('all', 'test_table', 'test_role', ARRAY['allowed']);
  $$,
  'verify_grant passes when all permissions granted to user on a specific column'
);

revoke all on table connectivity_engagement.test_table from test_role;
grant select (allowed) on table connectivity_engagement.test_table to test_role;

select lives_ok(
  $$
    select connectivity_engagement_private.verify_grant('select', 'test_table', 'test_role', ARRAY['allowed']);
  $$,
...
select throws_ok(
  $$
    select connectivity_engagement_private.verify_grant('bubbles', 'test_table', 'test_role');
  $$,
  'P0001',
