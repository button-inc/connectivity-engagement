-- Deploy connectivity_engagement:database_functions/verify_policy to pg
-- requires: schema_connectivity_engagement

begin;

-- Verifies if a policy exists
create or replace function connectivity_engagement_private.verify_policy(
  operation text,
  policy_name text,
...
comment on function connectivity_engagement_private.verify_policy(text, text, text, text, text) is
