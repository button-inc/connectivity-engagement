-- Deploy connectivity_engagement:util_functions/grant_permissions to pg

begin;

-- Grants permission for all columns in a table
create or replace function connectivity_engagement_private.grant_permissions(
  operation text,
  table_name text,
  role_name text,
  table_schema text default 'connectivity_engagement'
)
returns void
as
$function$
...
comment on function connectivity_engagement_private.grant_permissions(text, text, text, text) is
...
comment on function connectivity_engagement_private.grant_permissions(text, text, text, text[], text) is
